#!/usr/bin/env python3
"""Static audit for the deliberately constrained Python runtime."""
...
def main() -> None:
    ...
    for path in ROOT.rglob("*"):
        if ".git" in path.parts or "dist" in path.parts or "runs" in path.parts:
            continue
        if path.is_file() and (
            path.suffix.lower() in FORBIDDEN_SUFFIXES or path.name in FORBIDDEN_NAMES
        ):
    ...
    for path in sorted(RUNTIME.glob("*.py")):
        ...
        for root in imported_roots(tree):
            if root != "heresy_sec" and root not in sys.stdlib_module_names:
    ...
    print(
        "PASS standard-library Python; no frontend, dynamic execution, pickle, "
        "shell=True, hidden entropy, or third-party runtime imports"
    )
